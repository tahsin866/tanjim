...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div
      ...
      :key="index"
      class="bg-white rounded-lg shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow duration-300"
    >
        ...
        <div>
          ...
          <div class="flex items-center">
            ...
            <span class="ml-2 text-xs font-medium px-2 py-0.5 rounded-full"
              :class="stat.change.includes('+') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
              {{ stat.change }}
            </span>
          </div>
          <p class="text-gray-500 text-xs mt-1">গত মাসের তুলনায়</p>
        </div>
